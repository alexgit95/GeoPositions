...
public class MainVerticle extends AbstractVerticle {
	...
	private void getPositionsByMonth(RoutingContext ctx) {
		List<GeoPosition> positionsByMonth = daoServices.getPositionsByMonth(
				Integer.parseInt(ctx.request().getParam("year")), Integer.parseInt(ctx.request().getParam("month")));
	...
	private void getPositionsByPosition(RoutingContext ctx) {
		List<GeoPosition> positionsByPosition = daoServices.getPositionsByPosition(
				Double.parseDouble(ctx.request().getParam("lattitude")),
				Double.parseDouble(ctx.request().getParam("longitude")));
	...
	private void outputError(RoutingContext ctx, Exception e) {
		log.error("Erreur : ", e);
		e.printStackTrace();
		getJSONResponse(ctx).end(new JsonObject().put("status", "ko").put("msg", e.getMessage()).encodePrettily());
	}
